...
# Patch SmolVLM2 to use CPU with 8-bit quantization
class CPUSmolVLM2(SmolVLM2):
    def __init__(self, model_path="HuggingFaceTB/SmolVLM2-256M-Video-Instruct", use_quantization=False, **kwargs):
        ...
        self.resolution = 256  # Reduced for faster processing on Jetson CPU
        ...
        if use_quantization:
            logger.info("Applying 8-bit dynamic quantization")
...
# Patch Moondream2 to use CPU with 8-bit quantization
class CPUMoondream2(Moondream2):
    def __init__(self, model_path="vikhyatk/moondream2", revision="2025-01-09", use_quantization=False, **kwargs):
        try:
...
class CustomVLMBenchmark:
    def __init__(self, work_dir: str = './outputs', force_cpu: bool = True, dataset_sample_size: int = None, use_quantization: bool = False):
        """Initialize the benchmark with output directory.
        
        Args:
            work_dir (str): Directory to store outputs
            force_cpu (bool): Whether to force CPU execution (default True)
            dataset_sample_size (int, optional): Number of samples to use from the dataset
            use_quantization (bool): Whether to use 8-bit quantization for the model (default False)
        """
    ...
    def run_benchmark(self, model_name: str, dataset_name: str) -> Dict[str, Any]:
        ...
        logger.info(f"Running benchmark for {model_name} on {dataset_name} using "
                    ...
                    f"with {'8-bit quantization' if self.use_quantization else 'float32'}")
        ...
        try:
            ...
            torch.set_default_dtype(torch.float32)  # Updated from set_default_tensor_type
            ...
            if model_name == 'SmolVLM2-256M':
                model = CPUSmolVLM2(model_path="HuggingFaceTB/SmolVLM2-256M-Video-Instruct", use_quantization=self.use_quantization)
            elif model_name == 'Moondream2':
                model = CPUMoondream2(model_path="vikhyatk/moondream2", use_quantization=self.use_quantization)
            else:
            ...
            if self.dataset_sample_size is not None:
                ...
                subset_size = min(self.dataset_sample_size, total_samples)
...
def main():
    """Main function to run the benchmark on CPU with optional 8-bit quantization."""
    # Manually set model, dataset, and quantization
    model_name = 'SmolVLM2-256M'
    dataset_name = 'MMBench_DEV_EN'
    dataset_sample_size = 10  # Reduced for faster testing
    ...
    logger.info(f"Starting benchmark for {model_name} on {dataset_name} on CPU using "
                ...
                f"with {'8-bit quantization' if use_quantization else 'float32'}...")
    
    # Initialize benchmark
    benchmark = CustomVLMBenchmark(dataset_sample_size=dataset_sample_size, use_quantization=use_quantization)
    ...
    if 'error' in result:
        ...
        logger.info(f"Quantization: {'8-bit' if result['use_quantization'] else 'float32'}")
