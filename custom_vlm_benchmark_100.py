...
class CustomVLMBenchmark:
    ...
    def run_benchmark(self, model_name: str, dataset_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Special handling for MMVet evaluation
            if dataset_name == 'MMVet':
                logger.info("Initializing judge model for MMVet evaluation...")
                try:
                    # Initialize judge model for MMVet evaluation
                    judge_kwargs = {
                        'model': 'gpt-4-turbo',  # Use GPT-4 Turbo for MMVet evaluation
                        'max_tokens': 3,
                        'nproc': 4
                    }
                    judge_model = build_judge(**judge_kwargs)
                    if not judge_model.working():
                        raise RuntimeError("Failed to initialize judge model for MMVet evaluation. " + DEBUG_MESSAGE)
                    
                    # Evaluate results with judge model
                    logger.info("Evaluating results with judge model...")
                    eval_results = dataset.evaluate(str(result_path), model=judge_model)
                except Exception as e:
                    logger.error(f"Error during MMVet evaluation: {str(e)}")
                    raise
            else:
                # Normal evaluation for other datasets
...
def main():
    ...
    model_name = 'SmolVLM2-256M'  # Choose from: 'InternVL2_5-4B-MPO', 'SmolVLM2-256M', 'Moondream2'
    dataset_name = 'MMBench_DEV_EN'  # Choose from: 'MMBench_DEV_EN', 'SEEDBench_IMG', 'MME', 'MMStar', 'MMVet'
    dataset_sample_size = 100  # Use 100 samples from the dataset
